...
class HTTPServer(object):
    ...
    def start(self, num_processes=1, wait=True, fork_callback=None):
    ...
    def _wrap_parse_error(self, callback):
        def inner(*args, **kwargs):
            try:
                ...
                logging.warning("400 error parsing request", exc_info=True)
                headers = ("HTTP/1.1 400 Bad Request\r\n"
                           "Connection: Close\r\n\r\n")
                self.stream.write(headers, callback=self.stream.close)
        return inner
